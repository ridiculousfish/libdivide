  ...
  auto outer = [&](const auto integerdummy) -> int {
    auto inner = [&](const auto branchingtypedummy) -> int {
      ...
      const Integer divisor = [&]() {
        static_assert(sizeof(Integer) <= NbytesForDivisor,
                      "make NbytesForDivisor larger");
        Integer tmp;
      ...
      using ArrayOfIntegers =
        std::array<Integer, NbytesOfInput / sizeof(Integer)>;
      ...
      const Vector num_as_simdvector =
        _mm256_loadu_si256((const Vector*)numerators.data());
#endif
